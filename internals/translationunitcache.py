...
class TranslationUnitCache(Worker):
    ...
    def reparse(self, filename, unsaved_files=[], on_done=None):
        ret = False
        pl = self.parsingList.lock()
        try:
            if filename not in pl:
                ...
                self.tasks.put((
                    self.task_reparse,
                    (filename, self.get_opts(), unsaved_files, on_done)))
        finally:
            self.parsingList.unlock()
        return ret
    ...
    def add(self, filename, on_done=None):
        ret = False
        tu = self.translationUnits.lock()
        pl = self.parsingList.lock()
        try:
    ...
    def get_opts(self):
        ...
        if not get_setting("dont_prepend_clang_includes", False):
        ...
        cache = self.__options_cache.lock()
        try:
            cache[key] = (get_setting("options", []), opts)
        finally:
            self.__options_cache.unlock()
